...
impl SecurityPolicy {
    ...
    /// Atomically update the hot-reloadable security policy fields.
    /// Called by the gateway PUT /api/config handler.
...
mod tests {
    ...
    /// Build a policy with custom allowed-commands and forbidden-paths,
    /// using defaults for everything else. Keeps tests concise after the
    /// ArcSwap migration.
    fn policy_with(
        ...
        SecurityPolicy {
            autonomy,
            allowed_commands: Arc::new(ArcSwap::from_pointee(allowed_commands)),
            forbidden_paths: Arc::new(ArcSwap::from_pointee(forbidden_paths)),
            ..SecurityPolicy::default()
        }
    }
    ...
    fn from_config_maps_all_fields() {
        ...
        assert_eq!(**policy.allowed_commands.load(), vec!["docker"]);
        assert_eq!(**policy.forbidden_paths.load(), vec!["/secret"]);
